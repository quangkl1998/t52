...
export const getDetailSubMenu = createAsyncThunk(
    "submenu/getDetailSubMenu",
    async (body: any) => {
        try {
            const data = await submenuAPI.getDetailSubMenu(body);
            return data;
        } catch (error) {
            throw error;
        }
...
const submenuSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(getDetailSubMenu.pending, (state) => {
            state.isloading = true;
        });

        builder.addCase(getDetailSubMenu.fulfilled, (state, { payload }) => {
            state.isloading = false;
            state.detail = payload;
        });

        builder.addCase(getDetailSubMenu.rejected, (state, { error }) => {
            state.isloading = true;
            state.error = error.message;
        });
