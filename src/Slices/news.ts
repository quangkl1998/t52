...
export const getRandomNews = createAsyncThunk(
    ...
    async () => {
        try {
            const listNew = newsAPI.getRandomNews();
            return listNew;
        } catch (error) {
            throw error;
        }
...
const newsSlice = createSlice({
    ...
    extraReducers(builder) {
        ...
        builder.addCase(getRandomNews.pending, (state) => {
            state.isLoading = true;
        });
        builder.addCase(getRandomNews.fulfilled, (state, { payload }) => {
            state.isLoading = false;
            state.listNewRandom = payload;
        });
        builder.addCase(getRandomNews.rejected, (state, { error }) => {
            state.isLoading = false;
            state.error = error.message;
        });
